    ...
    : QQuickItem(parent), mPlay(false), mBoundChanged(true), mTextureChanged(true)
{
    ...
    connect(mCamera, &Camera::finished, this, &CameraPlayer::changeChannel);
	mCamera->start();
}
...
void CameraPlayer::setPlay(bool value)
{
    ...
    qDebug() << "bbbbbbbbbbbbbbbbbbbbbbbbb  setPlay";
    mCamera->stopStream();
...
void CameraPlayer::changeChannel()
{
    qDebug() << "cccccccccccccccccccccc  changeChannel";
//    sleep(3);
    if (mPlay) {
        mCamera->m_playChannel = PLAY_CHANNEL_1;
        mCamera->start();
    } else {
